...
void init_ethernet (void) 
{
  ...
  LPC_PINCON->PINSEL3 = (LPC_PINCON->PINSEL3 & ~0x0000000F) | 0x00000005;  //MDIO y MCLK operativos
...
static void write_PHY (U32 PhyReg, U16 Value) {
  ...
  LPC_EMAC->MADR = LAN8720_DEF_ADR | PhyReg;   //The device and registernumber that will be accessed		 
...
static U16 read_PHY (U32 PhyReg) {
  ...
  LPC_EMAC->MCMD = MCMD_READ;  //Set bit in the command register to execute a single read
